'use client';

import { Star } from 'lucide-react';
import FadeIn from '@presentation/components/ui/FadeIn';
import SectionHeading from '@presentation/components/ui/SectionHeading';
import { NEON } from '@presentation/constants';
import styles from './Reviews.module.css';

/* Ícono G de Google inline */
const GoogleG = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" aria-label="Google">
    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
  </svg>
);

export default function Reviews({ reviews, gym }) {
  return (
    <section style={{ background: '#000', padding: '7rem 0' }}>
      <div className={styles.inner}>

        <SectionHeading
          eyebrow="Lo que dicen"
          title="CLIENTES QUE"
          highlight="LO VIVIERON"
          sub={`${gym.rating} estrellas · ${gym.reviewCount} reseñas verificadas en Google`}
        />

        <div className={styles.grid}>
          {reviews.map((r, i) => (
            <FadeIn key={r.id} delay={i * 0.1}>
              <article
                className={styles.card}
                style={{ background: '#0D0D0D', border: '1px solid rgba(255,255,255,0.07)' }}
              >
                {/* Stars */}
                <div className={styles.stars}>
                  {[...Array(r.rating)].map((_, j) => (
                    <Star key={j} size={15} fill={NEON} stroke="none" />
                  ))}
                </div>

                {/* Quote */}
                <p className={styles.quote}>"{r.text}"</p>

                {/* Footer */}
                <div className={styles.cardFooter}>
                  <div className={styles.author}>
                    <span className={styles.avatar}
                      style={{ background: `${NEON}30`, color: NEON }}>
                      {r.author.charAt(0).toUpperCase()}
                    </span>
                    <div>
                      <p className={styles.authorName}>{r.author}</p>
                      <p className={styles.date}>{r.date}</p>
                    </div>
                  </div>
                  <GoogleG />
                </div>

                {r.isLocalGuide && (
                  <span
                    className={[styles.guideBadge, 'bc'].join(' ')}
                    style={{ background: `${NEON}15`, color: NEON, border: `1px solid ${NEON}30` }}
                  >
                    ⭐ Local Guide
                  </span>
                )}
              </article>
            </FadeIn>
          ))}
        </div>

      </div>
    </section>
  );
}
